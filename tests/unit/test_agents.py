...
@pytest.mark.asyncio
@patch("app.routing.intent_router.call_gemini", new_callable=AsyncMock)
async def test_intent_routing_risk(mock_call_gemini):
    """Test intent routing for risk analysis."""
    from app.routing.intent_router import route_intent

    mock_call_gemini.return_value = {"intent": "risk", "chunk_levels": ["meso", "macro"]}
...
@pytest.mark.asyncio
@patch("app.routing.intent_router.call_gemini", new_callable=AsyncMock)
async def test_intent_routing_kpi(mock_call_gemini):
    """Test intent routing for KPI extraction."""
    from app.routing.intent_router import route_intent

    mock_call_gemini.return_value = {"intent": "kpi", "chunk_levels": ["micro", "meso"]}
...
@pytest.mark.asyncio
@patch("app.routing.intent_router.call_gemini", new_callable=AsyncMock)
async def test_intent_routing_summary(mock_call_gemini):
    """Test intent routing for summary."""
    from app.routing.intent_router import route_intent

    mock_call_gemini.return_value = {"intent": "summary", "chunk_levels": ["macro"]}
